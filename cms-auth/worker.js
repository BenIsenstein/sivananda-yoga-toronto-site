/**
 * OAuth handler logic — ported VERBATIM from the official Sveltia CMS Auth worker
 * so it tracks upstream and keeps its security review (CSRF cookie, domain
 * allow-list). Only the Cloudflare `export default { fetch }` wrapper is replaced
 * by a Node bootstrap in `server.js`.
 *
 * Upstream: https://github.com/sveltia/sveltia-cms-auth (src/index.js)
 *
 * Uses only Web-standard APIs (Request/Response/URL/fetch/crypto.randomUUID),
 * all native in Node 20+.
 */
 ...
 * @param {string} [args.errorCode] - Error code for localization in Sveltia CMS.
...
const handleAuth = async (request, env) => {
  ...
  return new Response('', {
    ...
    headers: {
      Location: authURL,
      // Cookie expires in 10 minutes; Use `SameSite=Lax` so the cookie is sent
      // by the browser after redirect
      'Set-Cookie':
...
/**
 * Route a Web-standard `Request` to the correct handler.
 * Mirrors the upstream worker's `fetch(request, env)` entry point.
 * @param {Request} request - HTTP request.
 * @param {{ [key: string]: string }} env - Environment variables.
 * @returns {Promise<Response>} HTTP response.
 */
export const handleRequest = async (request, env) => {
  ...
  if (method === 'GET' && pathname === '/') {
    return new Response('Sveltia CMS Auth broker is running.', {
